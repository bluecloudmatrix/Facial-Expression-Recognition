if (parallable === undefined) {
	var parallable = function (file, funct) {
		...
		return function () {
			...
			//console.log("b" + arguments.length); //show 1
...
var ccv = {
	...
	detect_objects : parallable("ccv.js", function (canvas, cascade, interval, min_neighbors) {
		if (this.shared !== undefined) {
			...
			var params = get_named_arguments(arguments, ["canvas", "cascade_face", "interval", "min_neighbors"]);
			//console.log(params);
			this.shared.canvas = params.canvas;
			this.shared.interval = params.interval;
			this.shared.min_neighbors = params.min_neighbors;
			this.shared.cascade = params.cascade_face;
			
			this.shared.scale = Math.pow(2, 1 / (params.interval + 1));
			//console.log(this.shared.scale); 1.148...
			this.shared.next = params.interval + 1;
			
			this.shared.scale_upto = Math.floor(Math.log(Math.min(params.canvas.width / params.cascade_face.width, params.canvas.height / params.cascade_face.height)) / Math.log(this.shared.scale));
			//console.log(this.shared.scale_upto); 11
			var i;
